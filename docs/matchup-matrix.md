...
Evidence status: **source implementation and Android runtime catalog verified; full paired parity unverified**. The iOS Swift sources were inspected on 2026-08-21, all 28 supplied iOS unit tests passed, Android unit tests/build passed, and 27 deterministic Android states were captured on the task AVD. The iOS project has no fixture/XCUITest catalog, so only the clean welcome baseline could be paired; no row is labelled fully `matched`.

Shared profile is `ko-KR`, light appearance, portrait, default text scale, a deterministic family named `우리 가족`, sender `엄마`, member `첫째`, unless a row states otherwise. Android captures are 1080×2424 PNGs from `emulator-5580`; iOS captures are 1206×2622 PNGs from the dedicated iPhone 17 simulator. Immutable hashes are recorded in `docs/screenshots/SHA256SUMS`.
...
| settings_notification_denied | notification row | denied CTA | denied | red `차단됨`, `iPhone 알림 설정 열기` | literal iOS copy retained for matchup | platform wording is intentionally literal, not an OS exception | product copy decision remains visible in catalog | runtime/High | Android captured; paired iOS fixture unavailable |
...
```bash
ANDROID_SERIAL=emulator-5580 scripts/capture-android-catalog.sh
...
shasum -a 256 docs/screenshots/ios/*.png docs/screenshots/android/*.png
```
